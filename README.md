...
`<chat>:<id>` — `GROUP:1746`, `-1001234567890:1746`. That's what every surface
...
resolve the originating chat from it (like `reply`): `tg react SUPPORT:156 👍`,
`tg delete SUPPORT:156`. Pass `--to <chat>` only to override (or if the message
...
confused: `MSG [SUPPORT -4000000001] Bob #SUPPORT:156: …`. To answer,
copy a token straight off that line — `tg reply SUPPORT:156 "<text>"` (by the
ref; resolves the chat + forum thread from history and threads the reply in
place) or `tg send -4000000001 "<text>"` (by the chat_id). Don't route a reply
by a type word like `group` — that's how a support `group` and a team
...
tg show -1001234567890:1746   # id-qualified
...
negative and a file named `-1001234567890_1746.ogg` starts with a dash — every
...
ref you'd reply to also fetches the whole thing: `tg show SUPPORT:156` prints the
