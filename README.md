...
arg is almost always an unset shell var (`tg send "hi" "$SUPPORT"` with
`$SUPPORT` undefined), and silently broadcasting a meant-for-one-person DM to
...
confused: `MSG [SUPPORT -4000000001] Bob #156: …`. To answer, copy a token
...
place) or `tg send "<text>" -4000000001` (by the chat_id). Don't route a reply
